...
struct Nod {
	...
	Nod(DataType val) :valoare{ val }, urmator{ nullptr } {};
	// initializarea nodului cu un element urmator
	Nod(DataType val, Nod* urm) :valoare{ val }, urmator{ urm } {};
...
template <typename DataType>
void Lista<DataType>::erase(int position) {
	auto nodCurent = capat;
	auto prev = capat;
	int contor = 0;
...
template <typename DataType>
size_t Lista<DataType>::size() const {
	auto nodCurent = capat;
	int length = 0;
...
template <typename DataType>
DataType& Lista<DataType>::operator[](size_t position) {
	auto nodCurent = capat;
	int length = 0;
...
template <typename DataType>
void Lista<DataType>::elibereazaNoduri() {
	auto nodCurent = capat;
	while (nodCurent != nullptr) {
...
public:
	...
	*/
	ListaIterator(Nod<DataType>* curent) :curent{ curent } {}
	...
	*/
	bool operator!=(const ListaIterator& other);
...
template <typename DataType>
ListaIterator<DataType> Lista<DataType>::begin() const {
	return ListaIterator<DataType>{ capat };

}

template <typename DataType>
ListaIterator<DataType> Lista<DataType>::end() const {
	return ListaIterator<DataType>{ nullptr };

}

template <typename DataType>
bool ListaIterator<DataType>::operator!=(const ListaIterator& other) {
	return curent != other.curent;

}

template <typename DataType>
void ListaIterator<DataType>::operator++() {
	curent = curent->urmator;

}

template <typename DataType>
DataType& ListaIterator<DataType>::operator*() {
