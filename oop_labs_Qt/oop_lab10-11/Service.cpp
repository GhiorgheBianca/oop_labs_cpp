...
void Service::deleteDisciplinaService(int id, const string& denumire, int ore_pe_saptamana, const string& tip, const string& cadru_didactic) {
	Disciplina disciplina{ id,denumire,ore_pe_saptamana,tip,cadru_didactic };
	validator.validate(disciplina);
...
bool Service::exist(const Disciplina& disciplina, const vector<Disciplina>& disciplinele) const {
	try {
		repo.findPosition(disciplina.getDenumire(), disciplina.getCadruDidactic(), disciplinele);
		return true;
	}
	catch (RepoException&) {
		return false;
