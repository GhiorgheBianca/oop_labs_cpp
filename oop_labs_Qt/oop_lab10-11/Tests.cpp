...
void testAdauga() {
	...
	try {
		repo.findPosition("a", "b", repo.getDiscipline());
		assert(false);
	}
...
void testStergere() {
	...
	try {
		repo.findPosition("c", "c", repo.getDiscipline());
		assert(false);
	}
	catch (const RepoException & e) {
		stringstream os;
	...
	try {
		repo.findPosition("a", "a", repo.getDiscipline());
		assert(false);
	}
	catch (const RepoException & e) {
		stringstream os;
	...
	try {
		repo.findPosition("d", "d", repo.getDiscipline());
		assert(false);
	}
	catch (const RepoException & e) {
		stringstream os;
...
void testCauta() {
	...
	assert(repo.getDiscipline()[repo.findPosition("a", "c", repo.getDiscipline())].getDenumire() == "a");
	assert(repo.getDiscipline()[repo.findPosition("a", "c", repo.getDiscipline())].getCadruDidactic() == "c");
	assert(repo.getDiscipline()[repo.findPosition("b", "d", repo.getDiscipline())].getDenumire() == "b");
	assert(repo.getDiscipline()[repo.findPosition("b", "d", repo.getDiscipline())].getCadruDidactic() == "d");
...
void testStergeService() {
	...
	try {
		service.deleteDisciplinaService(0, "", 0, "", "");
		assert(false);
	}
	catch (ValidateException&) {
		assert(true);
	...
	assert(service.getDiscipline().size() == 1);
	service.deleteDisciplinaService(1, "a", 6, "facultativa", "a");
...
void testUndo() {
	...
	service.updateDisciplinaService(1, "a", 4, "facultativa", "a");
	assert(service.getDiscipline().at(0).getTip() == "facultativa");

	service.undo();
	assert(service.getDiscipline().size() == 3);
	assert(service.getDiscipline().at(0).getDenumire() == "a");

	service.deleteDisciplinaService(1, "a", 4, "facultativa", "a");
	service.deleteDisciplinaService(2, "b", 4, "facultativa", "b");
	assert(service.getDiscipline().size() == 1);
	try {
		repo.findPosition("b", "b", repo.getDiscipline());
		assert(false);
	}
	catch (RepoException&) {
		assert(true);
...
void testCitire_Scriere_Fisier() {
	...
	reopen_repo.updateDisciplina(Disciplina{ 1,"a",4,"optionala","e" });
	assert(reopen_repo.getDiscipline().at(0).getId() == 1);
	assert(reopen_repo.getDiscipline().at(0).getDenumire() == "a");
	assert(reopen_repo.getDiscipline().at(0).getOre() == 4);
	assert(reopen_repo.getDiscipline().at(0).getTip() == "optionala");
	assert(reopen_repo.getDiscipline().at(0).getCadruDidactic() == "e");
	reopen_repo.deleteDisciplina(Disciplina{ 1,"a",4,"optionala","e" });
	try {
		reopen_repo.findPosition("a", "e", reopen_repo.getDiscipline());
		assert(false);
	}
	catch (RepoException&) {
		assert(true);
