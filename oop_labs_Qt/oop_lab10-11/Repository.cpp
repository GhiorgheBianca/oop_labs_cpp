...
void Repository::updateDisciplina(Disciplina disciplina) {
	...
	int contor = findPosition(disciplina.getDenumire(), disciplina.getCadruDidactic(), discipline);
...
bool Repository::exist(const Disciplina& disciplina, const vector<Disciplina>& disciplinele) const {
	try {
		findPosition(disciplina.getDenumire(), disciplina.getCadruDidactic(), disciplinele);
		return true;
	}
	catch (RepoException&) {
		return false;
...
int Repository::findPosition(const string denumire, const string profesor, const vector<Disciplina>& disciplinele) const {
	int contor = 0;
	for (const auto& disciplina : disciplinele) {
		if (disciplina.getDenumire() == denumire && disciplina.getCadruDidactic() == profesor) {
			return contor;
		}
		else {
			contor++;
	...
	throw RepoException("Nu exista disciplina cu denumirea: " + denumire + " si cadrul didactic: " + profesor);
}
...
void Repository::deleteDisciplina_fromContract(const Disciplina& disciplina, Contract& contract) {
	...
	int contor = findPosition(disciplina.getDenumire(), disciplina.getCadruDidactic(), contract.getDiscipline_dinContract());
